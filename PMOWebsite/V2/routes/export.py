...
def create_forms_workbook(forms):
    ...
    op_headers = ['Document Code', 'Date', 'Operation', 'Start KM', 'End KM', 'Unit', 'Progress', 'Map Number', 'Explanation']
    ...
    for form in forms:
        for op in form.construction_operations:
            ...
            ws_op.cell(row=op_row, column=8, value=op.explanation or '').border = thin_border
            op_row += 1
...
def create_single_form_workbook(form):
    ...
    # Operations headers
    op_headers = ["نوع عملیات", "کیلومتر شروع", "کیلومتر پایان", "واحد", "پیشرفت", "شماره نقشه", "توضیحات"]
    for col, header in enumerate(op_headers, 1):
        cell = ws.cell(row=current_row, column=col, value=header)
        cell.font = header_font
        cell.fill = header_fill
    ...
    for op in form.construction_operations:
        ...
        ws.cell(row=current_row, column=5).alignment = center_align
        ws.cell(row=current_row, column=6, value=op.explanation or '').border = thin_border
