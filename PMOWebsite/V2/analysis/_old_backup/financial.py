...
class FinancialAnalysis:
    ...
    def _get_equipment_rate(self, equipment_type, equipment_model=None):
        ...
        if not eq_config:
            return 0
        # Check for direct hourly_rate (flat structure)
        if 'hourly_rate' in eq_config:
            return eq_config.get('hourly_rate', 0)
        # Nested structure: try model name, then 'default'
    ...
    def get_daily_cost_breakdown(self, date=None):
        ...
        for eq in form.tools_equipments:
            ...
            rate = self._get_equipment_rate(eq.equipment_type, eq.equipment_model)
    ...
    def get_monthly_breakdown(self, year=None, month=None):
        ...
        if year is None or month is None:
            # Default to the month of the most recent form
            latest_form = self.db.session.query(DailyFormSubmission.form_date).filter(
                DailyFormSubmission.project_id == self.project_id,
                DailyFormSubmission.status == 'approved'
            ).order_by(DailyFormSubmission.form_date.desc()).first()
            
            if latest_form:
                if year is None:
                    year = latest_form.form_date.year
                if month is None:
                    month = latest_form.form_date.month
            else:
