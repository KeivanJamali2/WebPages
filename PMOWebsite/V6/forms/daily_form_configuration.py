...
def get_hr_total_days(position, project_code):
    """
    Get the total planned days per year for a human resource position.
    ...
    Returns:
        Total planned days per year, or 0 if not found
    """
    try:
        rates = get_human_resources_rates(project_code)
        return rates.get(position, {}).get("total_days_per_year", 0)
    except FileNotFoundError:
        return 0
