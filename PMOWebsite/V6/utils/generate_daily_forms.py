...
def generate_single_form(config, project_info, j_date, shift, project_name):
    ...
    # Use total_days_per_year from config to decide who's likely to be present
    # Positions with more total_days_per_year are more likely to show up on any given day
    active_positions = []
    for pos in positions:
        total_days_per_year = hr_rates.get(pos, {}).get("total_days_per_year", 0)
        if total_days_per_year <= 0:
            # Low chance for unconfigured positions
            if random.random() < 0.15:
                active_positions.append(pos)
        else:
            # Probability = total_days_per_year / 365, capped at 0.95
            prob = min(total_days_per_year / 365.0, 0.95)
    ...
    for position in positions:
        ...
        if position in active_positions:
            ...
            total_days_per_year = hr_rates.get(position, {}).get("total_days_per_year", 0)
            if total_days_per_year <= 120:
                count = 1
            elif total_days_per_year <= 365:
    ...
    op_headers = ["نوع عملیات", "ایستگاه شروع (00+000)", "ایستگاه پایان (00+000)", "واحد", "مقدار", "توضیحات"]
    op_widths = [25, 20, 20, 18, 15, 25]
    ...
    # Randomly decide which materials arrive today
    active_mats = set()
