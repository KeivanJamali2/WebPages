...
def load_users_from_file(filepath):
    ...
    Full: username,password,phone_number,national_id,email,role
    """
    users = {}
...
def can_edit_form(form, user):
    ...
    # Only the submitter can edit, and only if draft or rejected
    if form.submitted_by_user.username != user['username']:
        return False
    return form.status in ['draft', 'rejected']
