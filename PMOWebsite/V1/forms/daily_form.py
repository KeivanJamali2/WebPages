...
class Tools_And_Equipments:
    def __init__(self):
        self.columns = ["tools_and_equipment", "count_active", "working_hours", "situation", "inactivity_reason"] # ماشین الات، تعداد فعال، ساعت کار، وضعیت، دلیل غیرفعال
        self.data = pd.DataFrame(columns=self.columns)
...
class Construction_Operations:
    def __init__(self):
        self.columns = ["operation_type", "start_km", "end_km", "unit", "progress", "map_number"] # نوع عملیات، کیلومتر شروع، کیلومتر پایان، واحد، مقدار پیشرفت، شماره نقشه
        self.data = pd.DataFrame(columns=self.columns)
        
    def add_item(self, d: list) -> None:
        "d -> 6 elements (operation_type, start_km, end_km, unit, progress, map_number)"
        self.data.loc[len(self.data)] = d
        
class Incoming_Materials_And_Goods:
    def __init__(self):
        self.columns = ["material_type", "incoming_amount", "cumulative_incoming", "used_amount", "cumulative_used", "storage_place", "waybill_number"] # نوع مصالح، مقدار ورودی، مجموع ورودی، مقدار مصرفی، مجموع مصرفی، محل نگهداری، شماره بارنامه
        self.data = pd.DataFrame(columns=self.columns)

    def add_item(self, d: list) -> None:
        "d -> 7 elements (material_type, incoming_amount, cumulative_incoming, used_amount, cumulative_used, storage_place, waybill_number)"
        self.data.loc[len(self.data)] = d
...
class Project_Issues:
    def __init__(self):
        self.columns = ["issue_type", "effect", "location_km", "start_time", "end_time", "notes"] # نوع مشکل، تاثیر، محل کیلومتر، زمان شروع، زمان خاتمه، توضیحات
        self.data = pd.DataFrame(columns=self.columns)
        
    def add_item(self, d: list) -> None:
        "d -> 6 elements (issue_type, effect, location_km, start_time, end_time, notes)"
        self.data.loc[len(self.data)] = d
