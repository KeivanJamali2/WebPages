...
class ResourceAnalysis:
    ...
    def get_equipment_utilization(self):
        ...
        for eq_type, count in self.equipment_config.items():
            planned_qty = self.equipment_costs.get(eq_type, {}).get('planned_quantity', 0)
            result[eq_type] = {
                'total_hours': 0,
                'total_count': 0,
    ...
    def get_planned_vs_actual_resources(self):
        ...
        equipment_result = {}
        for eq_type, config in self.equipment_costs.items():
            planned_hours = config.get('planned_quantity', 0)  # Planned total hours
            actual_hours = 0
        ...
        hr_result = {}
        for role, count in self.hr_config.items():
            actual_hours = 0
            actual_count = 0
