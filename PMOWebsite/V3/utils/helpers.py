...
def generate_document_code(project_code, username, date_obj, sequence=1):
    """Generate a unique document code for forms using Jalali date.
    
    Format: PROJECT_CODE-USERNAME-DATE-SEQUENCE
    Example: PROJ1-keivan-14040907-001
    
    Args:
        project_code: The project's code
        username: The username of the person creating the form
        date_obj: The date object (Gregorian) or date string
        sequence: Sequence number for multiple forms by same person on same day
    """
    if isinstance(date_obj, str):
    ...
    return f"{project_code}-{username}-{date_str}-{sequence:03d}"
