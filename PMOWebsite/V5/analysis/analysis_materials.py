...
class MaterialsAnalysis:
    ...
    Provides:
    - Summary table per material (incoming, used, stock, planned %)
    - Stock alerts
    """
    ...
    def __init__(self, data_service, project_config: dict):
        ...
        self.mat_prices = project_config.get('Material_Prices', {})
    ...
    def get_summary_table(self, unit: str = "per_day") -> Dict[str, Any]:
        ...
        planned_total, incoming_pct_plan, usage_pct_incoming,
        avg_incoming (unit-adjusted), avg_used (unit-adjusted).
        """
        ...
        for mt, d in by_mat.items():
            ...
            # Config
            price_info = self.mat_prices.get(mt, {})
            planned_total = price_info.get('planned_total', 0)

            incoming_pct_plan = round(total_incoming / planned_total * 100, 1) if planned_total else 0
            ...
            rows.append({
                ...
                'incoming_pct_plan': incoming_pct_plan,
