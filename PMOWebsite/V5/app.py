...
def create_app(config_name=None):
    ...
    with app.app_context():
        ...
        # Migrate: rename location_km to location_station in project_issues table
        try:
            from sqlalchemy import inspect, text
            inspector = inspect(db.engine)
            if 'project_issues' in inspector.get_table_names():
                columns = [c['name'] for c in inspector.get_columns('project_issues')]
                if 'location_km' in columns and 'location_station' not in columns:
                    db.session.execute(text('ALTER TABLE project_issues RENAME COLUMN location_km TO location_station'))
                    db.session.commit()
                elif 'location_km' not in columns and 'location_station' not in columns:
                    db.session.execute(text('ALTER TABLE project_issues ADD COLUMN location_station VARCHAR(20)'))
                    db.session.commit()
