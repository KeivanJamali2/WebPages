...
class SingleAtGradeCalculator(BaseCulvertCalculator):
    ...
    def get_template_filename(self) -> str:
        ...
        if self.params.alpha == 0:
            return f"single_perpendicular_{end}.dxf"
        elif self.params.alpha > 0:
            return f"single_angularM_{end}.dxf"
        else:
            return f"single_angularL_{end}.dxf"

    # ===================================================================== #
    #  TABLE 1 APPLICATION
    # ===================================================================== #

    def _apply_table1(self, d: pd.Series, check=False) -> Optional[str]:
        """
        ...
        H1 = self.data["c04"] - self.data["c06"] + 0.3
        H2 = self.data["c14"] - self.data["c12"] + 0.3
        ...
        if H1 > d["H"]:
            ...
            self.data["c07"] = round(self.data["c06"] - d["m"], 2)
            self.data["c04"] = round(self.data["c06"] + d["H"] - 0.3, 2)
        ...
        if H2 > d["H"]:
            ...
            sign = 1 if self.left == "up" else -1
            extra_width = hs * cos_alpha / ((1/self.params.u_shirvani + (sign) * self.params.z_natural / 100) * sqrt(1 + (self.params.z_natural/100)**2))
            self.data["c09"] = round(self.data["c09"] + extra_width, 2)
            ...
            self.data["c11"] = round(self.data["c12"] - d["m"], 2)
            self.data["c14"] = round(self.data["c12"] + d["H"] - 0.3, 2)
            
            print(f"temp: {temp}\nhs:{hs}\nsign:{sign}\nextra_width:{extra_width}\n")
            print(f"[INFO] Right UnderGrade Done!")
            self.is_undergrade = True
    
        self.data["c10"] = round((self.data["c08"] + self.data["c09"]), 1)
