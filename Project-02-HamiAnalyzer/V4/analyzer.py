...
class SQLiteAnalyzer:
    ...
    def __init__(self, db: 'HamiDatabase', plot_folder: Path, csv_folder: Path,
                 mapping_data_file_path: Path = None,
    ...
    def people_index(self) -> dict:
        """Load people_index.csv and return as dict {hami_id: full_name}."""
        if self._people_index is None:
            self._people_index = {}
            if self.mapping_data_file and self.mapping_data_file.exists():
                try:
                    df = pd.read_csv(self.mapping_data_file, index_col=0)
                    self._people_index = df['full_name'].to_dict()
                except Exception as e:
                    logger.warning(f"Could not load people_index: {e}")
    ...
    def top_communicators_for_employee(self, if_plot: bool = False, n: int = 10):
        ...
        senders_m, receivers_m = self.db.get_employee_stats_proper(by_messages=True, start_date=self.start_date_str, end_date=self.end_date_str)
        ...
        senders_r, receivers_r = self.db.get_employee_stats_proper(by_messages=False, start_date=self.start_date_str, end_date=self.end_date_str)
    ...
    def top_communicators_for_student(self, if_plot: bool = False, n: int = 10):
        ...
        senders_m, receivers_m = self.db.get_student_stats_proper(by_messages=True, start_date=self.start_date_str, end_date=self.end_date_str)
        ...
        senders_r, receivers_r = self.db.get_student_stats_proper(by_messages=False, start_date=self.start_date_str, end_date=self.end_date_str)
    ...
    def top_communicators_for_place(self, if_plot: bool = False, n: int = 10):
        ...
        # Get by messages
        senders_m, receivers_m = self.db.get_place_stats(by_messages=True, start_date=self.start_date_str, end_date=self.end_date_str)
        ...
        # Get by requests
        senders_r, receivers_r = self.db.get_place_stats(by_messages=False, start_date=self.start_date_str, end_date=self.end_date_str)
    ...
    def _plot_communicators(self, receivers_m, receivers_r, senders_m, senders_r,
        ...
        for ax, data, title, ylabel in data_sets:
            if len(data) > 0:
                ax.bar(range(len(data)), data.values, color='lightblue', edgecolor='darkblue', alpha=0.7)
    ...
    def total_messages_per_student(self, plot: bool = False):
        ...
        counts = self.db.get_student_message_counts(start_date=self.start_date_str, end_date=self.end_date_str)
        ...
        if plot:
            ...
            for ax, hist, title, color in [
                (axes[0], sent_hist, 'Messages Sent per Student', 'skyblue'),
                (axes[1], received_hist, 'Messages Received per Student', 'salmon')
            ]:
    ...
    def common_titles(self, n: int = 20, plot_: bool = False) -> pd.Series:
        """Get most common subjects - direct SQL query."""
        
        counts = self.db.get_common_subjects(start_date=self.start_date_str, end_date=self.end_date_str)
        result = pd.Series(counts).sort_values(ascending=False)
        result_n = result.head(n)
        
        result.to_csv(self.csv_path / 'common_titles.csv', index=True, encoding="utf-8-sig")
        
        if plot_:
            fig, ax = plt.subplots(figsize=(12, 6))
            
            bars = ax.bar(range(len(result_n)), result_n.values, 
                         color='skyblue', edgecolor='navy', alpha=0.7)
            
            for i, v in enumerate(result_n.values):
                ax.text(i, v + 0.5, str(v), ha='center', fontsize=12, fontweight='bold')
            ...
            ax.set_xticks(range(len(result_n)))
            ax.set_xticklabels([reshape_text(str(lbl)) for lbl in result_n.index], rotation=45, ha='right')
            ax.grid(True, axis='y', linestyle='--', alpha=0.7)
    ...
    def response_time_per_person(self, plot: bool = False):
        ...
        data = self.db.get_response_times_data(start_date=self.start_date_str, end_date=self.end_date_str)
        ...
        if plot:
            ...
            ax1.set_ylabel('Average Response Time (hours)', fontsize=10)
            ax1.set_xticks(range(len(avg_first_response_series)))
            ax1.set_xticklabels(avg_first_response_series.index, rotation=45, ha='right', fontsize=9)
            ax1.grid(True, axis='y', linestyle='--', alpha=0.5)
            
            # Add value labels on bars
            for i, (idx, v) in enumerate(avg_first_response_series.items()):
                days = v / 24
                ax1.annotate(f'{v:.1f}h\n({days:.1f}d)', 
                           xy=(i, v), ha='center', va='bottom', fontsize=8,
                           color='darkred')
            ...
            ax2.set_title('Average Request Duration per Hami\n(Time from first message to last message)', 
                         fontsize=12, fontweight='bold')
            ax2.set_ylabel('Average Duration (hours)', fontsize=10)
            ax2.set_xlabel('Hami ID', fontsize=10)
            ax2.set_xticks(range(len(avg_duration_series)))
            ax2.set_xticklabels(avg_duration_series.index, rotation=45, ha='right', fontsize=9)
            ax2.grid(True, axis='y', linestyle='--', alpha=0.5)
            
            # Add value labels on bars
            for i, (idx, v) in enumerate(avg_duration_series.items()):
                days = v / 24
                ax2.annotate(f'{v:.1f}h\n({days:.1f}d)', 
                           xy=(i, v), ha='center', va='bottom', fontsize=8,
                           color='navy')
    ...
    def place_filtered(self):
        ...
        self.place_data = self.db.get_place_request_details()
    ...
    def place_filtered_by(self, by: str = "field", top_n: int = 20, plot: bool = True):
        ...
        if by == "field":
            result = self.db.get_requests_count_by_field(start_date=self.start_date_str, end_date=self.end_date_str)
        elif by == "year":
            result = self.db.get_requests_count_by_year(start_date=self.start_date_str, end_date=self.end_date_str)
        elif by == "educational_level":
            result = self.db.get_requests_count_by_education_level(start_date=self.start_date_str, end_date=self.end_date_str)
        else:
            result = self.db.get_requests_count_by_field(start_date=self.start_date_str, end_date=self.end_date_str)
    ...
    def _add_reference_table(self, ref_ax, max_rows: int = 25, 
        ...
        # Get all hami IDs in the database and show their names
