...
class StoreData:
    ...
    """
    def __init__(self, base_output_path: Path, base_input_path: Path, date_source: str = "first", db: HamiDatabase = None):
    ...
    def fit(self, i_values: list[int]):
        ...
        print(f"  - Total attempted: {total_new + total_replaced + total_skipped} files")
    ...
    def _store_data(self, hami_id: str, number: int, combined_data: pd.DataFrame,
        ...
        Returns:
            'success' if stored as new, 'replaced' if existing data was replaced
        """
