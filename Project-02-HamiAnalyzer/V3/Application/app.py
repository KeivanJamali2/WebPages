    ...
    def run(self):
        try:
            ...
            scraper = HamiScraperByReferenceCode(
                ...
                i_value=self.config.get('i_value_rf', 0),
                name_value=self.config.get('name_value_rf', ''),
...
class HamiScraperApp(QMainWindow):
    ...
    def create_login_card(self):
        ...
        self.url_input.editingFinished.connect(lambda: self.save_config(show_message=False))
        
        self.username_input = card.add_field("Username", ModernLineEdit("your@email.com"))
        self.username_input.editingFinished.connect(lambda: self.save_config(show_message=False))
        
        self.password_input = card.add_field("Password", ModernLineEdit("••••••••"))
        self.password_input.setEchoMode(QLineEdit.Password)
        self.password_input.editingFinished.connect(lambda: self.save_config(show_message=False))
    ...
    def create_date_card(self):
        ...
        self.start_date.dateChanged.connect(lambda: self.save_config(show_message=False))
        ...
        self.end_date.dateChanged.connect(lambda: self.save_config(show_message=False))
    ...
    def create_scraper_card(self):
        ...
        self.i_value_input.editingFinished.connect(lambda: self.save_config(show_message=False))
        
        self.name_value_input = card.add_field("Name Value (Manual)", ModernLineEdit("حامی 001 واحد یزد"))
        self.name_value_input.editingFinished.connect(lambda: self.save_config(show_message=False))
    ...
    def create_path_card(self):
        ...
        self.chrome_path_input.editingFinished.connect(lambda: self.save_config(show_message=False))
        ...
        self.output_dir_input.editingFinished.connect(lambda: self.save_config(show_message=False))
    ...
    def create_timing_card(self):
        ...
        self.file_load_sleep_input.editingFinished.connect(lambda: self.save_config(show_message=False))
        ...
        self.workflow_load_sleep_input.editingFinished.connect(lambda: self.save_config(show_message=False))
        ...
        self.page_load_sleep_input.editingFinished.connect(lambda: self.save_config(show_message=False))
        ...
        self.outdate_tolerance_input.editingFinished.connect(lambda: self.save_config(show_message=False))
    ...
    def create_rf_search_card(self):
        ...
        self.rf_output_dir_input.editingFinished.connect(lambda: self.save_config(show_message=False))
        ...
        self.rf_search_wait_input.editingFinished.connect(lambda: self.save_config(show_message=False))
        ...
        # I Value for RF
        rf_i_container = QWidget()
        rf_i_layout = QVBoxLayout()
        rf_i_layout.setSpacing(4)
        rf_i_layout.setContentsMargins(0, 0, 0, 0)
        rf_i_label = QLabel("I Value")
        rf_i_label.setStyleSheet("font-size: 20px; font-weight: 500; color: #7F8C8D;")
        self.rf_i_value_input = ModernLineEdit("0")
        self.rf_i_value_input.setPlaceholderText("0")
        self.rf_i_value_input.editingFinished.connect(lambda: self.save_config(show_message=False))
        rf_i_layout.addWidget(rf_i_label)
        rf_i_layout.addWidget(self.rf_i_value_input)
        rf_i_container.setLayout(rf_i_layout)
        
        rf_config_layout.addWidget(rf_output_container)
        rf_config_layout.addWidget(search_wait_container)
        rf_config_layout.addWidget(rf_i_container)
    ...
    def get_rf_config(self):
        ...
        config.update({
            ...
            'i_value_rf': int(self.rf_i_value_input.text() or '0'),
            'search_wait_time': int(self.rf_search_wait_input.text() or '5'),
        })
        
        return config, reference_codes
    
    def save_rf_config(self, show_message=False):
        """Save RF config to JSON (as extension of main config)"""
        try:
            ...
            if show_message:
                self.log("✅ RF Configuration saved successfully!")
        except Exception as e:
            if show_message:
                self.log(f"❌ Error saving RF configuration: {str(e)}")
    
    def load_rf_config(self):
        """Load RF config from JSON file"""
        if self.config_file.exists():
            try:
                ...
                self.rf_i_value_input.setText(str(config.get('i_value_rf', '0')))
    ...
    def start_rf_search(self):
        ...
        try:
            ...
            self.save_rf_config(show_message=False)
    ...
    def get_config(self):
        ...
        return {
            ...
            'i_value': int(self.i_value_input.text()),
    ...
    def start_scraping(self):
        ...
        try:
            ...
            # Auto-save configuration before starting (without showing message)
            self.save_config(show_message=False)
